...
Journey _journey({int delay = 0, bool cancelled = false}) => Journey(
  legs: [
    JourneyLeg(
      origin: _selent,
      destination: _kiel,
...
void main() {
  ...
  test(
    'on time: title is just the destination, chip counts down to the stop',
    () async {
  ...
  test('a direct trip: delay + arrival, no intermediate-stop metric', () async {
    await LiveUpdateService.show(_journey(delay: 12), now: _at(9, 10));
    final metrics = (lastPost()['metrics']! as List)
  ...
  test(
    'the current leg is split at the live position so the bar fills',
    () async {
      await LiveUpdateService.show(_journey(), now: _at(9, 10));
      final args = lastPost();
  ...
  test(
    'the countdown and text lead with the rider\'s OWN stop, not the trip end',
    () async {
      // ERX 83 Kiel 10:43 → Lüneburg 13:17 (the rider's transfer), passing
      // Lübeck ~12:14, then a second train on to Hamburg 14:00.
      final luebeck = const Station(
        id: '8000237',
        name: 'Lübeck Hbf',
        locationId: 'A=1@',
      );
      final lueneburg = const Station(
        id: '8000237b',
        name: 'Lüneburg',
        locationId: 'A=1@',
      );
      final hamburg = const Station(
        id: '8002549',
        name: 'Hamburg Hbf',
        locationId: 'A=1@',
      );
  ...
  test(
    'the icon carries the state: train, or train-alert when late (#76)',
    () async {
      await LiveUpdateService.show(_journey(), now: _at(9, 10));
