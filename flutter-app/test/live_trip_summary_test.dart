...
Journey _journey({int delay310 = 0, int delay22 = 0}) => Journey(
  legs: [
    _leg(
      from: _selent,
      to: _kiel,
...
void main() {
  group('the bar is the journey', () {
    ...
    test('progress follows the clock and never runs past the end', () {
      ...
      expect(
        summariseTrip(_journey(), _at(23, 0)).progressMinutes,
        47,
  ...
  group('the colours say what state each leg is in', () {
    ...
    test('everything ahead is pending before the trip starts', () {
      ...
      expect(
        trip.segments.every((s) => s.color == LiveTripColors.pending),
        isTrue,
      );
    });
  ...
  group('which train the rider is on', () {
    test('the one being ridden', () {
      expect(
        summariseTrip(_journey(), _at(9, 10)).currentLeg?.line?.name,
        'Bus 310',
      );
      expect(
        summariseTrip(_journey(), _at(9, 44)).currentLeg?.line?.name,
        'Bus 22',
      );
    });
  ...
  group('when there is nothing to draw', () {
    ...
    test('a journey without times cannot be drawn, and says so', () {
      final trip = summariseTrip(
        Journey(
          legs: [JourneyLeg(origin: _selent, destination: _kiel)],
        ),
        _at(9, 0),
      );
      expect(trip.isEmpty, isTrue);
    ...
    test('a finished trip is finished', () {
      expect(
        summariseTrip(_journey(), _at(10, 30)).finishedAt(_at(10, 30)),
        isTrue,
      );
      expect(
        summariseTrip(_journey(), _at(9, 30)).finishedAt(_at(9, 30)),
        isFalse,
      );
    });
