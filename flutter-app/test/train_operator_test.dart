...
void main() {
  group('operatorFor (#operator)', () {
    ...
    test('operator short in the line name is also picked up', () {
      expect(
        operatorFor(productName: '', product: 'regional', name: 'ME 82')?.name,
        'metronom',
      );
    });
    ...
    test('DB long-distance and regional fall back by product', () {
      expect(
        operatorFor(productName: 'ICE', product: 'nationalExpress')?.name,
        'DB Fernverkehr',
      );
      expect(
        operatorFor(productName: 'RE', product: 'regional')?.name,
        'DB Regio',
      );
      expect(
        operatorFor(productName: 'S', product: 'suburban')?.name,
        'DB Regio',
      );
    });
